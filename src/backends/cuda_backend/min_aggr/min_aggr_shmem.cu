...
// __device__ __forceinline__ void min_aggr_forward_small_impl(
//     int v,
//     int row_start,
//     int row_end,
...
__device__ __forceinline__ void min_aggr_forward_large_impl(
    int v,
    int row_start,
    int row_end,
    const int* __restrict__ edge_idx,
...
// __global__ void min_aggr_forward_light_kernel(
...
//     min_aggr_forward_small_impl(v, row_start, row_end, edge_idx, X, out, argmin, d);
// }
...
__global__ void min_aggr_forward_heavy_kernel(
    ...
    int row_start = edge_ptr[v];
    int row_end = edge_ptr[v + 1];
    min_aggr_forward_large_impl(v, row_start, row_end, edge_idx, X, out, argmin, d);
}
