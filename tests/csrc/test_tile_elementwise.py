"""VecOpsFloatBase elementwise ops, every (N, num_type) and both host and device.
...
def test_elementwise(bridge, op, n, dtype_name, on_host, kind):
    ...
    check_elementwise(got, ref, op, dtype_name, f"{op} {dtype_name} N={n} {kind} host={on_host}", paths)
...
@pytest.mark.parametrize(("n", "dtype_name"), COMBOS, ids=COMBO_IDS)
def test_binary_flavours_agree(bridge, base, flavour, n, dtype_name, on_host):
...
@pytest.mark.parametrize("on_host", [False, True], ids=["device", "host"])
@pytest.mark.parametrize(("n", "dtype_name"), COMBOS, ids=COMBO_IDS)
def test_leaky_relu_at_zero(bridge, n, dtype_name, on_host):
...
@pytest.mark.parametrize("on_host", [False, True], ids=["device", "host"])
@pytest.mark.parametrize(("n", "dtype_name"), COMBOS, ids=COMBO_IDS)
def test_leaky_relu_backward_boundary(bridge, n, dtype_name, on_host):
...
@pytest.mark.parametrize(("n", "dtype_name"), COMBOS, ids=COMBO_IDS)
def test_minmax_nan_propagation(bridge, op, n, dtype_name, on_host):
    """min/max do not agree on NaN between the host and device passes.
    ...
    picks_non_nan = (not on_host) or dtype_name in ("half", "bf16")
    if picks_non_nan:
        ...
        assert got.isnan().all(), (
            f"{op} in the host pass uses cuda::std::{op.rstrip('_')[:3]}, which propagates the NaN first operand"
        )
