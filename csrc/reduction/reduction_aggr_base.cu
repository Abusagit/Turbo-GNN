...
void reduction_aggr_backward_cuda(
    ...
    int blocks_per_sm = 8, int sched_chunk = 1, int bucket_launch = 0
);
...
at::Tensor reduction_aggr_backward_torch(
    ...
    TORCH_CHECK(
        grad_out.scalar_type() == at::kFloat || grad_out.scalar_type() == at::kHalf || grad_out.scalar_type() == at::kBFloat16,
        "grad_out must be float32/float16/bfloat16"
    );
    TORCH_CHECK(is_supported_index_type(arg_idx.scalar_type()), "arg_idx must be int32, int64, uint32, or uint64");
    ...
    reduction_aggr_backward_cuda(grad_out, arg_idx, grad_x, warps_per_block, schedule, blocks_per_sm, sched_chunk, bucket_launch);
    return grad_x;
}
