...
Тот же прогон turbo-против-DGL, но по всему набору: GraphLand (10), Planetoid
(cora, citeseer, pubmed) и OGB (arxiv, products, proteins). d=64, stages=0,
медиана из 2 прогонов по 30 запусков. Строит `run_matrix.sh`, сводит
`summarize.py`, графы готовит `prepare_graph.py` из одного кэша, который читают
обе стороны сравнения.

**Происхождение чисел, важное.** Мерялся не коммит, а снапшот исходников
`/home/ubuntu/work/turbo-bench/snap-0909_2329`: согласованное состояние
существовало только в рабочей копии, а прогон идёт полтора часа, за которые
дерево продолжали править. В снапшоте лежат `PROVENANCE_head.txt` (`a04c105`) и
`PROVENANCE_worktree.diff` (767 незакоммиченных строк в `csrc/` и `turbo_gnn/`).
Поэтому таблицы четырёхграфового раздела выше **не** заменены: у них есть хеш, а
у этих — нет.

При 16 графах картинка на конфигурацию — 183 полосы, поэтому основной вид здесь
«строка на граф»: [pergraph_fp32_backward.png](pergraph_fp32_backward.png),
[pergraph_fp32_forward.png](pergraph_fp32_forward.png),
[pergraph_fp16_backward.png](pergraph_fp16_backward.png),
[pergraph_fp16_forward.png](pergraph_fp16_forward.png). Подробные по клеткам —
всё те же `fp32_*.png` / `fp16_*.png`, но с 64 крайними полосами из 183.

## fp32

| граф | N | E | макс. вх. | перекос | клеток | forward | backward | fwd+bwd |
|---|---|---|---|---|---|---|---|---|
| `city-roads-M` | 57,073 | 189,643 | 7 | 2× | 18 | 2.13x | 1.86x | 1.96x |
| `pubmed` | 19,717 | 108,368 | 172 | 31× | 18 | 2.16x | 1.69x | 1.89x |
| `ogbn-arxiv` | 169,343 | 1,335,586 | 13,156 | 1668× | 18 | 2.06x | 1.59x | 1.87x |
| `tolokers-2` | 11,758 | 1,049,758 | 2,139 | 24× | 18 | 1.82x | 1.59x | 1.75x |
| `city-roads-L` | 142,257 | 421,318 | 10 | 3× | 18 | 1.64x | 1.56x | 1.60x |
| `artnet-exp` | 50,405 | 611,101 | 199 | 16× | 18 | 1.86x | 1.52x | 1.66x |
| `city-reviews` | 148,801 | 2,479,631 | 7,438 | 446× | 18 | 1.76x | 1.41x | 1.53x |
| `citeseer` | 3,327 | 12,555 | 100 | 26× | 18 | 2.37x | 1.38x | 1.60x |
| `cora` | 2,708 | 13,264 | 169 | 35× | 18 | 2.34x | 1.36x | 1.58x |
| `pokec-regions` ¹ | 1,632,803 | 32,255,367 | 13,734 | 695× | 3 | 1.18x | 1.09x | 1.12x |
| `twitch-views` ¹ | 168,114 | 13,763,228 | 35,280 | 431× | 3 | 1.29x | 1.04x | 1.21x |
| `ogbn-products` ¹ | 2,449,029 | 126,167,309 | 17,482 | 339× | 3 | 1.07x | 1.02x | 1.05x |
| `avazu-ctr` ¹ | 76,269 | 22,044,423 | 42,032 | 145× | 3 | 1.33x | 1.00x | 1.31x |
| `hm-categories` ¹ | 46,563 | 21,508,553 | 31,459 | 68× | 3 | 1.25x | 0.96x | 1.24x |
| `web-fraud` ¹ | 2,890,331 | 15,785,700 | 228,991 | 41928× | 3 | 1.69x | 0.94x | 1.31x |
| `ogbn-proteins` ¹ | 132,534 | 79,255,038 | 7,751 | 13× | 3 | 1.23x | 0.79x | 1.23x |

¹ только `copy_u`: операнд `[E, d]` и его градиент при таком числе рёбер не
влезают в 15 GB, и `run.sh` сам решает это по числу рёбер, а не по имени графа.
Geomean такого графа считается по 3 клеткам против 18 у остальных, поэтому
строки напрямую не сравниваются.

## fp16

| граф | N | E | макс. вх. | перекос | клеток | forward | backward | fwd+bwd |
|---|---|---|---|---|---|---|---|---|
| `avazu-ctr` ¹ | 76,269 | 22,044,423 | 42,032 | 145× | 3 | 1.28x | 7.02x | 2.02x |
| `web-fraud` ¹ | 2,890,331 | 15,785,700 | 228,991 | 41928× | 3 | 1.57x | 1.71x | 1.69x |
| `tolokers-2` | 11,758 | 1,049,758 | 2,139 | 24× | 18 | 2.09x | 1.59x | 1.85x |
| `artnet-exp` | 50,405 | 611,101 | 199 | 16× | 18 | 2.04x | 1.42x | 1.66x |
| `city-roads-M` | 57,073 | 189,643 | 7 | 2× | 18 | 1.91x | 1.40x | 1.61x |
| `city-reviews` | 148,801 | 2,479,631 | 7,438 | 446× | 18 | 1.85x | 1.37x | 1.58x |
| `pubmed` | 19,717 | 108,368 | 172 | 31× | 18 | 2.10x | 1.35x | 1.65x |
| `citeseer` | 3,327 | 12,555 | 100 | 26× | 18 | 2.32x | 1.32x | 1.55x |
| `ogbn-arxiv` | 169,343 | 1,335,586 | 13,156 | 1668× | 18 | 2.49x | 1.30x | 1.76x |
| `cora` | 2,708 | 13,264 | 169 | 35× | 18 | 2.25x | 1.26x | 1.49x |
| `city-roads-L` | 142,257 | 421,318 | 10 | 3× | 18 | 1.42x | 1.15x | 1.27x |
| `hm-categories` ¹ | 46,563 | 21,508,553 | 31,459 | 68× | 3 | 1.23x | 0.87x | 1.21x |
| `pokec-regions` ¹ | 1,632,803 | 32,255,367 | 13,734 | 695× | 3 | 1.00x | 0.86x | 0.95x |
| `twitch-views` ¹ | 168,114 | 13,763,228 | 35,280 | 431× | 3 | 1.35x | 0.85x | 1.21x |
| `ogbn-products` ¹ | 2,449,029 | 126,167,309 | 17,482 | 339× | 3 | 0.97x | 0.74x | 0.87x |
| `ogbn-proteins` ¹ | 132,534 | 79,255,038 | 7,751 | 13× | 3 | 1.25x | 0.63x | 1.22x |

## Что показал набор

**Разделение по размеру, а не по перекосу.** Все девять графов с полной таблицей
операций дают 1.36–1.86x в backward и 1.64–2.37x в forward. Все семь больших,
где идёт только `copy_u`, стоят на 0.79–1.09x. Перекос при этом ни при чём:
`city-roads-M` с перекосом 2× даёт лучший результат набора (1.86x), а
`ogbn-arxiv` с перекосом 1668× — 1.59x. Дело в том, что у больших графов
осталась одна операция, и это та самая, где мы слабее всего.

**Общий geomean, с оговоркой.** fp32: forward 1.65x, backward 1.26x. fp16:
forward 1.63x, backward 1.28x — но эта цифра завышена одним графом. На
`avazu-ctr` DGL в fp16 внезапно в 22.5 раза медленнее собственного fp32 на
backward min/max (0.56 → 9.70 мс), тогда как наше время почти не меняется
(0.67 → 0.77 мс). Отношение там вырастает до 7.02x и тянет средние вверх. Это
аномалия DGL, изолированная одним графом — на остальных пятнадцати отношение
fp16/fp32 у DGL равно 0.6–1.1x. **Без `avazu-ctr` fp16 backward — 1.14x, и это
честная цифра.**

**Единственный значимый проигрыш** — `copy_u/sum` backward на `web-fraud`:
33.5 мс против 25.1 у DGL, 0.75x. Тот же дефект виден на `city-roads-L` (0.73x),
но там он стоит 1.1 мс и практического значения не имеет. Причина не в ядрах:
`_split_by_quantile` в [graph.py](../../turbo_gnn/graph.py) порогует только по
`torch.quantile`, без абсолютного минимума, поэтому в heavy всегда уходит ~5%
вершин независимо от того, велики ли они. У `city-roads-L` 95-й процентиль
исходящей степени равен 4 при максимуме 10 — такие вершины попадают в 2-D
heavy-ядро, где 256 потоков делят 4 ребра на 8 плиток и ради этого ещё
выполняется редукция в разделяемой памяти. Механизм изолируется контролем:
`city-roads-M` — тот же тип графа и те же степени, но квантиль попала на
совпадение значений и в heavy ушло 56 вершин вместо 4 899, и он даёт 1.00x
вместо 0.73x. Абсолютный порог порядка `tiles_y × несколько рёбер` должен убрать
и то, и другое, ничего не сломав там, где heavy-bucket осмыслен.

**Что дали последние два раунда правок ядер** (`858bdb5`, `5cbb66a` и
незакоммиченное поверх), по `compare_runs.py` против набора от 09-09 утра:
forward geomean 0.99x, backward 1.02x при разбросе по графам ±8%, лучший
`city-roads-M` 1.22x, худший `citeseer` 0.93x. Контрольная колонка DGL — 1.00x
на всех графах, то есть наборы сопоставимы и это действительно измерение кода, а
не условий. Клетка `copy_u/sum` backward на `web-fraud` не изменилась
(34.4 → 33.5 мс), то есть порог buckets этими раундами не затронут.
