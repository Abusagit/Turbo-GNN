...
Таблицы и картинки ниже — из одного прогона. `results/*.json` под `results*` в
`.gitignore`, как и у DGL-прогона, так что в репозитории их нет; повторить:

```
cd dev/scanhex12/gspmm_before_after
for g in random skewed; do
  python bench.py results/before_$g.json --graph $g --label '603239d (PR как был)'   # на 603239d
  python bench.py results/after_$g.json  --graph $g --label '2dc800c (после правок)' # на 2dc800c
done
for k in bwd fwd; do
  python plot.py --kind $k -o ../fp16_${k}_before_after.png \
    --pair results/before_random.json results/after_random.json \
...
|---|---|---|---|
| `copy_e/sum` | 6.09 мс | 1.37 мс | **4.45x** |
| `add/sum` | 7.54 | 2.95 | **2.55x** |
| `mul/sum` | 13.53 | 5.85 | **2.31x** |
| `div/sum` | 14.33 | 7.21 | **1.99x** |
| `mul/min` | 12.74 | 7.90 | 1.61x |
| `div/min` | 12.72 | 7.90 | 1.61x |
| `copy_u/sum` | 1.57 | 1.57 | 1.00x (контроль) |
| `copy_u/max` | 3.98 | 3.96 | 1.01x (контроль) |

Forward на `random` не изменился нигде: все восемь клеток в пределах 0.98–1.00x.

## skewed (та же N и E, максимальная степень 66865)

| клетка | forward было | стало | backward было | стало |
|---|---|---|---|---|
| `copy_u/sum` | 2.34 мс | 1.81 (**1.29x**) | 1.38 мс | 1.37 (1.00x) |
| `copy_e/sum` | 2.39 | 1.49 (**1.61x**) | 10.43 | 1.36 (**7.69x**) |
| `add/sum` | 5.22 | 2.76 (**1.89x**) | 11.66 | 2.75 (**4.24x**) |
| `mul/sum` | 5.11 | 2.72 (**1.88x**) | 37.08 | 5.62 (**6.60x**) |
| `div/sum` | 5.00 | 2.64 (**1.90x**) | 42.16 | 6.96 (**6.06x**) |
| `mul/min` | 4.78 | 4.42 (1.08x) | 10.49 | 6.20 (1.69x) |
| `div/min` | 4.84 | 4.54 (1.07x) | 10.46 | 6.20 (1.69x) |
| `copy_u/max` | 3.88 | 3.60 (1.08x) | 3.01 | 3.02 (1.00x) |

Sum-клетки forward на `skewed` теперь в пределах 12% от `random` той же
размерности (2.72 против 2.45 у `mul/sum`), было — в 2.1 раза хуже. У min/max
осталось 4.4–4.5 против 2.6 у sum: это и есть незакрытый пункт про слайсинг для
...
- **Edge slicing для min/max.** Нужен packed-путь (`atomicMin`/`atomicMax` по
  ...
  (4.42 против 2.72 у sum-клеток) и только для 32-битных индексов.
