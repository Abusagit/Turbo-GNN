...
__constant__ float d_const_A[kMaxConstantMemorySize];

// Kernel 2: Vector A in constant memory
__global__ void SoftmaxDotProductConstantKernel(
    ...
    for (int k = thread_id; k < K; k += blockDim.x) {
        ...
        for (int i = num_float4 * 4; i < z; i++) {
            dot_product += d_vectors[k * vectors_stride + i] * d_const_A[i];
        }
...
void SoftmaxDotProductConstant(
    size_t K, size_t z,
    ...
    // Copy vector A to constant memory
    cudaMemcpyToSymbol(d_const_A, h_A, z * sizeof(float));
    ...
    SoftmaxDotProductConstantKernel<<<nBlocks, nThreads, shared_mem_size, stream>>>(
        K, z, d_vectors, d_logits, vectors_stride, d_out
    );
}
