...
__device__ __forceinline__ float leaky_relu_float4_gatv2(float4 left, float4 right, float negative_slope) {
...
__global__ void GATv2Kernel_CSR(
    ...
    float* __restrict__ d_alpha_out,        // [E] - attention weights per edge -- can be used for backward
    float negative_slope,
    bool use_constant_attn,
    int max_neighbors                       // Maximum neighbors NOTE this is cringe, for more neighbors we should use strided loop over neighbors
) {
    // shared memory layout:
    // [0, z): l_i cached
    // [z, z + max_neighbors): logits/alpha for this node's edges
    extern __shared__ float shared[];
    float* l_shared = shared;
    float* logits = l_shared + z;
    ...
    // рфтвду isolated nodes (no neighbors)
    if (num_neighbors == 0) {
        return;
    }

    const float4* attn_ptr = use_constant_attn ?
                             reinterpret_cast<const float4*>(d_const_attn_vec) :
                             reinterpret_cast<const float4*>(d_attn_vec);
    int num_float4 = z / 4;
    ...
    for (int k = 0; k < num_neighbors; ++k) {
        ...
        for (int i = lane_id; i < num_float4; i += kMaxThreadsInWarp) {
            ...
            // LeakyReLU
            sum.x = (sum.x > 0.0f) ? sum.x : negative_slope * sum.x;
            sum.y = (sum.y > 0.0f) ? sum.y : negative_slope * sum.y;
            sum.z = (sum.z > 0.0f) ? sum.z : negative_slope * sum.z;
            sum.w = (sum.w > 0.0f) ? sum.w : negative_slope * sum.w;
    ...
    for (int i = lane_id; i < num_neighbors; i += blockDim.x) {
    ...
    // optional: Write attention weights to edge array NOTE THIS IS PLACEHOLDER FOR BACKWARD
    if (d_alpha_out != nullptr) {
        for (int i = lane_id; i < num_neighbors; i += blockDim.x) {
            d_alpha_out[edge_start + i] = logits[i];
        }
    }
    ...
    // h_i = sum_j a_ij * r_j
    // ==========================================
    const float* alpha = logits;

    // register accumulators for output
    int float4_per_thread = (num_float4 + blockDim.x - 1) / blockDim.x;  // Ceiling division
    ...
    for (int k = 0; k < num_neighbors; ++k) {
        ...
        // Coalesced within warp, but scattered across nodes
        for (int i = 0; i < float4_per_thread; ++i) {
            int idx = lane_id + i * kMaxThreadsInWarp;
            if (idx < num_float4) {
                float4 r_val = r_ptr[idx];

                h_acc[i].x += alpha_k * r_val.x;
                h_acc[i].y += alpha_k * r_val.y;
                h_acc[i].z += alpha_k * r_val.z;
                h_acc[i].w += alpha_k * r_val.w;
            }
        }
    }
...
void GATv2Forward_CSR(
    ...
    size_t shared_mem_size = (z + max_neighbors) * sizeof(float);

    GATv2Kernel_CSR<<<nBlocks, nThreads, shared_mem_size, stream>>>(
        N, z, d_l, d_r, d_row_ptr, d_col_idx, d_attn_vec,
        d_h_out, d_alpha_out, negative_slope, false, max_neighbors
    );
}
